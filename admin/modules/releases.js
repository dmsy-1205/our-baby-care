import { escapeHtml } from '../admin-utils.js?v=admin-2-0-a12-action-guard-20260718';

const ADMIN_RELEASE_STEP = 'STEP A12';
const ADMIN_RELEASE_LABEL = 'Action Guard';
    ...
    <section class="module-view" aria-labelledby="adminReleaseHeading">
      <div class="foundation-notice">
        ...
        <div>
          ...
          <p>메인앱 기준과 관리자 콘솔 스텝을 분리해서 확인합니다. 이 화면은 읽기 전용입니다.</p>
        </div>
      </div>
      ...
      <div class="metric-grid admin-release-metrics">
        <article class="metric-card">
          <span>메인앱 기준</span>
          <strong>${escapeHtml(release.step || 'STEP6.2.13.4')}</strong>
          <small>${escapeHtml(release.appVersion || release.version || 'HearMe2nite v1.0')}</small>
        </article>
        <article class="metric-card">
          <span>관리자 기준</span>
        ...
        <article class="metric-card">
          <span>릴리스 날짜</span>
          <strong>${escapeHtml(release.releaseDate || '2026.07.18')}</strong>
        ...
        <div class="admin-release-grid">
          <section class="admin-release-card">
            ...
            <dl>
              ...
              <div><dt>앱 버전</dt><dd>${escapeHtml(release.appVersion || release.version || 'HearMe2nite v1.0 STEP6.2.13.4')}</dd></div>
              <div><dt>스텝</dt><dd>${escapeHtml(release.step || 'STEP6.2.13.4')}</dd></div>
          ...
          <section class="admin-release-card">
            ...
            <dl>
              ...
              <div><dt>실행 기능</dt><dd>없음 · 검토/상태 저장만 가능</dd></div>
            </dl>
          </section>
        </div>
      </article>
