...
import { escapeHtml, formatDateTime, compactId } from '../admin-utils.js?v=admin-2-0-a11-1-2-route-cleanup-20260719';
import { renderEmptyState } from '../components/empty-state.js?v=admin-2-0-a11-1-2-route-cleanup-20260719';
...
async function loadRecoveryRows() {
  ...
  Object.entries(requestsRoot).forEach(([ownerUid, requestGroup]) => {
    Object.entries(asObject(requestGroup)).forEach(([requestId, value]) => {
      ...
      rows.push({
        ...request, ownerUid, requestId, requestType,
        status: request.status || 'pending', roomCode,
  ...
  return `
    <article class="admin-card admin-recovery-card">
      <div class="admin-request-card-head">
        <div><h3>${escapeHtml(typeLabel(row.requestType))}</h3><p>${escapeHtml(row.email || row.ownerUid || '요청자 정보 없음')}</p></div>
        <span class="admin-status-pill muted">${escapeHtml(closed ? '종료 요청' : '열린 요청')}</span>
      </div>
      ...
      <div class="admin-grid admin-grid-3">
        <article class="admin-soft-card">
          ...
          <ul>
            <li>사용자 정보: ${yesNo(row.hasUser)}</li><li>사용자 Room 연결: ${yesNo(row.hasUserRoom)}</li>
            <li>Room 멤버십: ${yesNo(row.hasRoomMembership)}</li><li>Room 데이터: ${yesNo(row.hasRoomData)}</li>
            <li>Room 멤버 수: ${row.roomMemberCount}명</li>
          </ul>
        </article>
        <article class="admin-soft-card">
          <h4>확인 경로</h4>
          <ul><li>users/${escapeHtml(row.ownerUid)}</li><li>userRooms/${escapeHtml(row.ownerUid)}/${escapeHtml(room)}</li><li>roomMembers/${escapeHtml(room)}/${escapeHtml(row.ownerUid)}</li><li>rooms/${escapeHtml(room)}</li></ul>
        </article>
        <article class="admin-soft-card">
          <h4>실행 전 체크</h4>
          <ul><li>요청자 본인 요청과 현재 상태 확인</li><li>공동 Room 기록과 상대방 권리 보존 여부 확인</li><li>관리자 메모와 감사 로그 확인</li><li>실제 삭제는 별도 승인 단계에서만 진행</li></ul>
        </article>
      </div>
...
export async function render() {
  ...
  return `
    <section class="module-view" aria-labelledby="recoveryHeading">
      <section class="admin-hero-card"><div class="admin-hero-icon">🛟</div><div><h2 id="recoveryHeading">복구 센터 · 실행 전 기준점</h2><p>삭제나 Room 연결 해제 전에 되돌림 기준 정보를 읽기 전용으로 확인합니다. 이 단계에서는 데이터를 변경하지 않습니다.</p></div></section>
      <section class="admin-grid admin-grid-4">
        <article class="admin-card admin-metric"><span>기준 요청</span><strong>${rows.length}</strong><small>전체 데이터 요청</small></article>
