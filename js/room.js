...
이 설정은 앱 설치 순서나 방 생성 순서와 관계없으며, 최초 1회 선택 후 일반 화면에서는 변경할 수 없습니다.`);
    ...
    async function hmLoadRelationshipState(roomCode = activeRoomCode) {
        if (!currentUser || !hmIsSafeRoomCode(roomCode)) {
        ...
        try {
            const relationshipSnap = await db.ref(`rooms/${roomCode}/meta/relationship`).once('value');
    ...
    function hmStopRelationshipStateListener() {
        if (relationshipStateRef) relationshipStateRef.off();
        relationshipStateRef = null;
        relationshipStateRoomCode = '';
    }

    function hmStartRelationshipStateListener(roomCode = activeRoomCode) {
        if (!currentUser || !hmIsSafeRoomCode(roomCode)) return;
        if (relationshipStateRef && relationshipStateRoomCode === roomCode) return;
        hmStopRelationshipStateListener();
        const sessionUid = currentUser.uid;
        relationshipStateRoomCode = roomCode;
        ...
        relationshipStateRef.on('value', (snapshot) => {
            ...
            activeRelationshipState = snapshotValue
                ? hmNormalizeRelationshipState(snapshotValue, roomCode)
                : hmNormalizeRelationshipState(activeRelationshipState && activeRelationshipState.roomCode === roomCode ? activeRelationshipState : null, roomCode);
            ...
            if (activeRelationshipStatus !== 'active') {
                ...
                hmRefreshPresenceFromRoom('relationship-realtime-recovered');
                showSaveStatus('💞 관계 회복 · 데이터 연결 완료');
            }
        }, (error) => {
    ...
    function hmRenderRelationshipLifecycle() {
        const box = document.getElementById('relationshipLifecycleBox');
        if (!box) return;
        const state = activeRelationshipState || hmNormalizeRelationshipState(null);
        const hasPartner = Boolean(state.partnerUid);
        box.hidden = !activeRoomCode || !hasPartner;
        box.dataset.status = state.status;
        const badge = document.getElementById('relationshipLifecycleBadge');
        const message = document.getElementById('relationshipLifecycleMessage');
        const endBtn = document.getElementById('endRelationshipBtn');
        const requestBtn = document.getElementById('requestRecoveryBtn');
        ...
        if (approveBtn) approveBtn.hidden = state.status !== 'recovery_pending' || requestedByMe;
        if (cancelBtn) cancelBtn.hidden = state.status !== 'recovery_pending' || !requestedByMe;
    }
    ...
    async function hmEndRoomRelationship() {
        if (!currentUser || !activeRoomCode || activeRelationshipStatus !== 'active') return;
        if (hmIsAutoSaving || uploadedPhotoBase64 || (Array.isArray(hmDailyMoments) && hmDailyMoments.length > 0)) {
            alert('저장 또는 사진 작업이 진행 중입니다. 작업을 완료하거나 취소한 뒤 관계를 종료해 주세요.');
            return;
        }
        const warning = '관계를 종료하면 Dom과 Sub 모두 기존 기록, 사진, 채팅, 코멘트와 알림을 불러오거나 새로 작성할 수 없습니다.\n\n데이터는 삭제되지 않으며 두 사람이 관계 회복에 동의해야 다시 열 수 있습니다.';
        if (!confirm(warning)) return;
        const typed = prompt('실수 방지를 위해 “관계 종료”를 정확히 입력해 주세요.');
        if (typed !== '관계 종료') {
            if (typed !== null) alert('확인 문구가 일치하지 않아 취소했습니다.');
            return;
        }
        const state = activeRelationshipState;
        const nextState = {
            status: 'ended',
    ...
    async function hmRequestRoomRecovery() {
        if (!currentUser || activeRelationshipStatus !== 'ended') return;
        if (!confirm('상대방에게 관계 회복 동의를 요청할까요? 상대방이 동의하기 전까지 데이터는 계속 잠깁니다.')) return;
        const state = activeRelationshipState;
        await hmWriteRelationshipState({
            status: 'recovery_pending',
            ownerUid: state.ownerUid,
            partnerUid: state.partnerUid,
            endedByUid: state.endedByUid,
            endedAt: state.endedAt,
            generation: Number(state.generation || 0),
            recoveryRequestedByUid: currentUser.uid,
            recoveryRequestedAt: firebase.database.ServerValue.TIMESTAMP
        }, '💞 관계 회복 동의 대기 중');
    }

    async function hmApproveRoomRecovery() {
        const state = activeRelationshipState;
        if (!currentUser || activeRelationshipStatus !== 'recovery_pending' || state.recoveryRequestedByUid === currentUser.uid) return;
        if (!confirm('관계 회복에 동의하면 Dom과 Sub 모두 이 공간의 기존 데이터를 다시 불러오고 새 기록을 작성할 수 있습니다. 동의할까요?')) return;
        const nextState = {
            status: 'active',
            ownerUid: state.ownerUid,
            partnerUid: state.partnerUid,
            recoveredByUid: currentUser.uid,
            recoveredAt: firebase.database.ServerValue.TIMESTAMP,
            generation: Number(state.generation || 0) + 1
        };
        if (await hmWriteRelationshipState(nextState, '💞 관계 회복 완료')) {
            setDataSectionsVisible(true);
            connectAndListenFirebase();
            hmRefreshPresenceFromRoom('relationship-recovered');
            updateCurrentRoomInfo();
        }
    }

    async function hmCancelRoomRecovery() {
        const state = activeRelationshipState;
        if (!currentUser || activeRelationshipStatus !== 'recovery_pending' || state.recoveryRequestedByUid !== currentUser.uid) return;
        if (!confirm('관계 회복 요청을 취소할까요? 데이터는 잠긴 상태로 유지됩니다.')) return;
        await hmWriteRelationshipState({
            status: 'ended',
            ownerUid: state.ownerUid,
            partnerUid: state.partnerUid,
    ...
    function updateCurrentRoomInfo() {
        ...
        if (activeRoomCode) {
            ...
            // 정책:
            // - owner: 새 방 만들기, 초대코드 생성, 초대코드 입력, 참여했던 방 재연결 가능
    ...
    async function hmRequireRoomAccess(actionLabel = '작업', roomCode = getRoomCodeForData()) {
        ...
        if (activeRelationshipStatus !== 'active') {
            resetProtectedDataUI('관계가 종료되어 이 공간의 데이터가 잠겼습니다.');
            showSaveStatus(`🔒 관계 회복 후 ${actionLabel}할 수 있습니다.`);
            return false;
        }
        return true;
    }
    ...
    async function loadMyRoomList() {
        ...
        try {
            ...
            for (const roomCode of roomCodes) {
                ...
                const relationshipSnap = await db.ref(`rooms/${roomCode}/meta/relationship`).once('value');
                if (!currentUser || currentUser.uid !== uid) return;
                const relationship = hmNormalizeRelationshipState(relationshipSnap.val(), roomCode);
                let meta = {};
            ...
            box.innerHTML = items.map(item => {
                ...
                const lifecycleLabel = item.relationshipStatus === 'ended' ? ' · 관계 종료됨' : item.relationshipStatus === 'recovery_pending' ? ' · 회복 동의 대기' : '';
                const disabled = item.isActive ? 'disabled style="opacity:.55; cursor:default;"' : '';
                const btnText = item.isActive ? '현재 공간' : item.relationshipStatus === 'active' ? '이 공간 열기' : '관계 상태 보기';
                return `
                    <div class="room-list-item">
                        <div class="room-list-title">💕 ${escapeHtml(item.roomCode)}</div>
                        <div class="room-list-meta">${roleLabel}${activeLabel}${lifecycleLabel}<br>생성/참여일: ${formatTimestamp(item.createdAt || item.joinedAt)}</div>
    ...
    function getRoomCodeForData() {
        return activeRelationshipStatus === 'active' ? (activeRoomCode || '') : '';
    }
...
코드가 만료되면 방 주인에게 새 코드를 요청해 주세요.`;
    ...
    async function acceptInviteCodeValue(code, fromPending) {
        ...
        try {
            ...
            const relationshipSnap = await relationshipRef.once('value');
            if (!relationshipSnap.exists()) {
                await relationshipRef.set({
                    status: 'active',
                    ownerUid: initialInvite.ownerUid,
