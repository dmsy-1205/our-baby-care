...
(function(){
    ...
    function hmRenderSummarySettings(){
        ...
        const byKey=new Map(HM_SUMMARY_CATALOG.map(item=>[item.key,item])),orderedItems=hmSummaryDraft.map(key=>byKey.get(key)).filter(Boolean).concat(HM_SUMMARY_CATALOG.filter(item=>!hmSummaryDraft.includes(item.key))),rows=orderedItems.map(item=>{ const index=hmSummaryDraft.indexOf(item.key), selected=index>=0; return `<div class="hm-summary-setting-row${selected?' is-selected':''}"><label><input type="checkbox" ${selected?'checked':''} data-hm-change="toggle-summary-item" data-hm-value="${item.key}"><span>${item.icon}</span><strong>${safe(item.label)}</strong></label><div>${selected?`<button type="button" data-hm-action="move-summary-item" data-hm-value="${item.key}" data-hm-number="-1" aria-label="${safe(item.label)} 위로" ${index===0?'disabled':''}>↑</button><button type="button" data-hm-action="move-summary-item" data-hm-value="${item.key}" data-hm-number="1" aria-label="${safe(item.label)} 아래로" ${index===hmSummaryDraft.length-1?'disabled':''}>↓</button>`:''}</div></div>`; }).join('');
