...
/* RC2 v2.6.0 STEP3: removed duplicate earlier `displayHistory()` implementation; final implementation below is authoritative. */
    ...
    function openHistoryDetailModal(date) {
        ...
        content.innerHTML = `
            ...
            <button type="button" class="history-detail-copy" onclick="copyDirectText(event, '${hmEscapeInlineJs(date)}')">📋 이 기록 복사하기</button>
        `;
...
/* RC2 v2.6.0 STEP3: removed duplicate earlier `displayHistory()` implementation; final implementation below is authoritative. */
    ...
    function renderCalendar(daysData) {
        ...
        for (let day = 1; day <= last.getDate(); day++) {
            ...
            html += `<div class="calendar-day ${rec ? 'has-record' : ''} ${ymd === todayYmd ? 'today' : ''} ${ymd === selectedHistoryDate ? 'selected-record' : ''}" ${rec ? `onclick="selectHistoryDate('${hmEscapeInlineJs(ymd)}')"` : ''}>${day}<span class="calendar-icons">${icons}</span></div>`;
        }
        html += '</div>';
        box.innerHTML = html;
    }
    ...
    function renderHistoryPhotoGalleryModal() {
        ...
        if (grid) {
            grid.innerHTML = visibleDates.map(date => {
                ...
                return `<button type="button" class="history-photo-modal-item" onclick="closeHistoryPhotoGallery(); openHistoryDetailModal('${hmEscapeInlineJs(date)}')" aria-label="${escapeHtml(formatHistoryDateLabel(date))} 사진 기록 열기">
    ...
    const selectedCard = selectedRecord ? (() => {
        ...
        return `<button type="button" class="history-day-card history-premium-selected history-selected-record-card" onclick="openHistoryDetailModal('${hmEscapeInlineJs(selectedHistoryDate)}')">
            ...
            <span class="history-day-actions">
                ...
                <span class="btn-delete" onclick="deleteRecord(event, '${hmEscapeInlineJs(selectedHistoryDate)}')">삭제</span>
            </span>
        </button>`;
    ...
    const recentCards = filtered.slice(0, 10).map(date => {
        ...
        return `<button type="button" class="history-story-row ${isSelected ? 'is-selected' : ''}" onclick="selectHistoryDate('${hmEscapeInlineJs(date)}')">
...
async function openHistoryDetailModal(date) {
    ...
    content.innerHTML = `
        ...
        <div class="history-detail-actions">
            <button type="button" class="history-detail-copy" onclick="copyDirectText(event, '${hmEscapeInlineJs(date)}')">📋 이 기록 복사하기</button>
            <button type="button" class="history-detail-delete" onclick="deleteRecord(event, '${hmEscapeInlineJs(date)}')">삭제</button>
        </div>`;
    openModalOverlayById('historyDetailOverlay');
}
...
function renderCalendar(daysData) {
    ...
    for (let day = 1; day <= last.getDate(); day++) {
        ...
        const clickable = visible ? `onclick="selectHistoryDate('${hmEscapeInlineJs(ymd)}')"` : '';
