...
// =========================================================
(function () {
    const HM_PWA_APP_VERSION = 'v1.0-step6-2-13-6';
    const HM_PWA_SW_URL = '/service-worker.js?v=step6-2-13-6-admin-data-requests-actions-20260718';
    ...
    function ensureInstallBanner() {
        ...
        installBanner.setAttribute('aria-label', 'HearMe2nite ???§Ïπò ?àÎÇ¥');
        installBanner.innerHTML = `
            <button type="button" class="hm-pwa-install-main" aria-label="HearMe2nite ???§Ïπò?òÍ∏∞">
                <span aria-hidden="true">?íó</span>
                <b>${isIOSDevice() ? '?ÑÏù¥?????îÎ©¥??Ï∂îÍ?' : '?±ÏúºÎ°??§Ïπò?òÍ∏∞'}</b>
            </button>
            <button type="button" class="hm-pwa-install-close" aria-label="???§Ïπò ?àÎÇ¥ ?´Í∏∞">√ó</button>
        `;
    ...
    function ensureIOSGuideOverlay() {
        ...
        iosGuideOverlay.setAttribute('aria-label', '?ÑÏù¥?????îÎ©¥ Ï∂îÍ? ?àÎÇ¥');
        iosGuideOverlay.innerHTML = `
            <div class="hm-pwa-ios-guide-card">
                <button type="button" class="hm-pwa-ios-guide-close" aria-label="?´Í∏∞">√ó</button>
                <span class="hm-pwa-ios-guide-icon" aria-hidden="true">?ì≤</span>
                <strong>?ÑÏù¥?∞Ï? ÏßÅÏ†ë Ï∂îÍ??¥Ïïº ?¥Ïöî</strong>
                <p>Safari ?òÎã®??<b>Í≥µÏú† Î≤ÑÌäº</b>???ÑÎ•∏ ??br><b>???îÎ©¥??Ï∂îÍ?</b>Î•??†ÌÉù?òÎ©¥ ?±Ï≤ò???????àÏñ¥??</p>
                <ol>
                    <li>Safari?êÏÑú HearMe2nite ?¥Í∏∞</li>
                    <li>Í≥µÏú† Î≤ÑÌäº ?ÑÎ•¥Í∏?/li>
                    <li>???îÎ©¥??Ï∂îÍ? ?†ÌÉù</li>
                </ol>
                <button type="button" class="hm-pwa-ios-guide-ok">?åÍ≤†?¥Ïöî</button>
            </div>
        `;
    ...
    async function promptInstall() {
        if (!deferredInstallPrompt) {
            if (isIOSDevice()) {
                ...
                showToast('Î∏åÎùº?∞Ï? Î©îÎâ¥?êÏÑú ???îÎ©¥??Ï∂îÍ??????àÏñ¥??');
                rememberDismiss(1);
                hideInstallBanner();
            }
            return;
        ...
        try {
            ...
            if (choice && choice.outcome === 'accepted' && typeof showToast === 'function') {
                showToast('HearMe2niteÍ∞Ä ?±Ï≤ò???§Ïπò?©Îãà?? ?íú');
            } else {
                rememberDismiss(1);
            }
        } finally {
    ...
    window.addEventListener('appinstalled', () => {
        ...
        hideInstallBanner();
        if (typeof showToast === 'function') showToast('HearMe2nite ?§ÏπòÍ∞Ä ?ÑÎ£å?êÏñ¥?? ?íú');
    });
