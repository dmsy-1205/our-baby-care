...
function renderSubRoutine(){
    const rows=hmSubRoutineRows(); const done=rows.filter(r=>hmSubRoutineChecks?.[r.id]?.done===true).length;
    const sub=document.getElementById('subRoutineHubSub'); const count=document.getElementById('subRoutineCountText'); const list=document.getElementById('subRoutineList');
    if(count) count.textContent=hmCanManageSubRoutine()?`${rows.length}개 · ${done}/${rows.length} 완료 · 기록(Sub)가 관리`:`${rows.length}개 · ${done}/${rows.length} 완료 · Dom은 확인만`;
    if(sub) sub.textContent=!hmSubRoutineRoomCode?'공간을 연결하면 나의 루틴을 사용할 수 있어요.':!rows.length?(hmCanManageSubRoutine()?'나만의 하루 루틴을 만들어보세요.':'Sub가 루틴을 만들면 여기에 표시됩니다.'):`${rows.length}개 루틴 · ${done}/${rows.length} 완료`;
    const add=document.getElementById('subRoutineAddBtn'); if(add)add.hidden=!hmCanManageSubRoutine();
    const note=document.getElementById('subRoutineRoleNote'); if(note)note.textContent=hmCanManageSubRoutine()?'내가 직접 만들고 체크하는 개인 루틴입니다.':'기록(Sub)가 직접 관리하는 루틴입니다. Dom은 완료 상태만 확인할 수 있습니다.';
    if(!list)return;
