...
## STEP5.6.1.6 운영자 삭제 요청 관리
- [ ] 일반 계정에는 `관리` 버튼이 보이지 않는다.
- [ ] `admins/{uid}` 등록 계정에만 `관리` 버튼이 보인다.
- [ ] 관리자는 전체 삭제 요청 목록을 불러올 수 있다.
- [ ] 검토 중/보류/승인/거절 상태 변경이 저장된다.
- [ ] 사용자 답변이 사용자 요청 내역에 표시된다.
...
## STEP5.6.1.9.1 사용자 요청 취소
- [ ] 접수됨 상태에서 삭제 요청 취소 가능
- [ ] 검토 중 상태에서 삭제 요청 취소 가능
- [ ] 보류 상태에서 삭제 요청 취소 가능
- [ ] 승인 상태에서는 취소 버튼 미표시
- [ ] 취소 후 사용자 화면에 `사용자 취소` 표시
- [ ] 관리자 전체 목록에서 취소 이력 확인
- [ ] 취소 요청 기록 자체는 유지
